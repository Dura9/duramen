...
export default function Auth() {
  ...
  async function handleSubmit(e) {
    ...
    if (mode === 'register') {
      ...
      if (error) setError(error.message)
      else setMessage('Un email de réinitialisation a été envoyé. Vérifie ta boîte mail.')
    } else {
      ...
      <div className="card fade-in" style={{ padding: 28 }}>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <button type="submit" className="btn-primary" disabled={loading} style={{ marginTop: 8 }}>
            {loading ? <span className="spinner"></span> : mode === 'login' ? 'Se connecter' : mode === 'register' ? 'Créer mon compte' : 'Envoyer le lien de réinitialisation'}
          </button>
        </form>
